...
def _init_training_state(
    ...
    ppo_optimizer: optax.GradientTransformation) -> TrainingState:
  """Inits the training state and replicates it over devices."""
  key_policy, key_value = jax.random.split(key)
  
  init_ppo_params = ppo_losses.PPONetworkParams(
      policy=ppo_network.policy_network.init(key_policy),
      value=ppo_network.value_network.init(key_value),
  )

  ppo_optimizer_state = ppo_optimizer.init(init_ppo_params)

  normalizer_params = running_statistics.init_state(
      specs.Array((obs_size,), jnp.dtype('float32')))

  training_state = TrainingState(
      ppo_optimizer_state=ppo_optimizer_state,
      ppo_params=init_ppo_params,
      env_steps=jnp.zeros(()),
...
def train(
  ...
  replay_buffer = replay_buffers.UniformSamplingQueue(
      ...
      sample_batch_size=batch_size * grad_updates_per_step // device_count) # change this to 2
  

  ppo_loss = functools.partial(
  ...
  def prefill_replay_buffer(
    ...
    def f(carry, unused):
      ...
      new_normalizer_params, env_state, buffer_state = get_experience(
          training_state.normalizer_params, training_state.ppo_params.policy,
  ...
  params = _unpmap(
      (training_state.normalizer_params, training_state.ppo_params))
