...
# change training_state to ppo training state
...
# This is actually - fine replace with ppo components
...
def train(
  ...
  # replace with ppo network factory
  ...
  replay_buffer = replay_buffers.UniformSamplingQueue(
      ...
      sample_batch_size=batch_size * grad_updates_per_step // device_count) # Should be 2 

  # change to ppo losses
  ...
  # functions:
  # I should be able to keep the same functions and same parameters + return values
  # obv change logic to ppo
  def sgd_step(
  ...
  # keep the get_experience call but remove sample (this is done outside)
  def training_step(
    ...
    normalizer_params, env_state, buffer_state = get_experience( # don't call get experience, instead gen_unroll if possible? (can insert into buffer)
