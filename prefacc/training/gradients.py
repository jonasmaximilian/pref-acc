...
def loss_and_pgrad(loss_fn: Callable[..., float],
  ...
  def h(*args, **kwargs):
    ...
    jax.debug.print("Raw gradients: {g}", g=grad)
...
def gradient_update_fn(loss_fn: Callable[..., float],
  ...
  def f(*args, optimizer_state):
    ...
    # Debug print for value
    jax.debug.print("Loss value: {v}", v=value)
    
    # Check if gradients are zero and log detailed information
    grad_norm = jax.tree_util.tree_reduce(lambda x, y: x + jax.numpy.sum(y**2), grads, 0.0)
    jax.debug.print("Gradient norm: {x}", x=grad_norm)
    
    # Print individual gradients
    jax.tree_map(lambda g: jax.debug.print("Gradient: {g}", g=g), grads)
    
    # # Add gradient clipping to prevent exploding gradients
    # grads = jax.tree_map(lambda g: jax.numpy.clip(g, -1.0, 1.0), grads)
